...
NSATS=31;
 for numsat=1:1:NSATS
     ...
     % **** QUESTION 2 ****:
     % Open the source code of digitGPS_L1_CA.m and codegen.m.
     % Explain what is the purpose of each function.
     % What is the relationship between the codes generated by both functions?
     
    s_sat_tx(numsat,:)=digitGPS_L1_CA(num_chips,Fs,0,numsat,1); %PRN GPS_l1_ca muestreada a Fs
 end
...
% null of the GPS L1 CA modulation, each side.What parameter controls these frequencies?
figure;
subplot(2,1,1)
...
title('Transmitted baseband signal (QP2026) Corrected');
xlabel('Hz');
ylabel('dBW/Hz');
...
%s_rf_tx=?
s_rf_tx = Asig * s_bb_tx .* cos(2*pi*Fc*t);
...
% s_rf_rx=?
s_rf_rx = Asig * s_bb_tx .* cos(2*pi*(Fc + Fd)*t);
...
Y_rf_tx_spectrum_db2=10*log10((abs([Y_rf_tx(floor((NFFT/2))+1:1:end) Y_rf_tx(1:1:(NFFT/2))]).^2)/(Nsamples*Fs));
...
Ps_estim = (1/Nsamples) * (s_rf_rx * s_rf_rx');
Pn_estim = (1/Nsamples) * (noise * noise');

SNR_estim_dB = 10*log10(Ps_estim/Pn_estim);
...
% y_bb_rx_I_filt=?
% y_bb_rx_Q_filt=?
y_bb_rx_I = 2 * y_rf_rx .* cos(2*pi*Fc*t);
y_bb_rx_Q = -2 * y_rf_rx .* sin(2*pi*Fc*t);

y_bb_rx_I_filt = filtfilt(b,a,y_bb_rx_I);
y_bb_rx_Q_filt = filtfilt(b,a,y_bb_rx_Q);
...
% surface of 42 dB-Hz. Set the obtained value in SNR_dB in the noise
...
SNR_dB = CN0_nominal_dBHz - 10*log10(RF_BW_Hz);

disp(['Nominal real GPS SNR is ' num2str(SNR_dB) ' [dB]']);
